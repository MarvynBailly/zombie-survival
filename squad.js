...
// World survivors are stored in Game.worldSurvivors; recruited ones go to
// Game.squad. Permadeath — they drop a notice with their name and remove.

const SQUAD_CAP = 3;
const SQUAD_LEASH = 90;
const SQUAD_TIGHT_RANGE = 60;
const SQUAD_TELEPORT_DIST = 600;
...
const SQUAD_CLASS = {
  medic: {
    color: '#8ec547', label: 'MEDIC',
    hp: 70, speed: 180, auraR: 80, healPerSec: 1.0,
    update(s, dt) {
      const r2 = s.auraR * s.auraR;
      const p = Game.player;
      if (p && !p.dead && p.hp < p.maxHp) {
        const dx = p.x - s.x, dy = p.y - s.y;
        if (dx * dx + dy * dy <= r2) p.hp = Math.min(p.maxHp, p.hp + s.healPerSec * dt);
      }
      for (const m of Game.squad) {
        if (m === s || m.hp >= m.maxHp) continue;
  ...
  engineer: {
    color: '#e3a83a', label: 'ENG',
    hp: 80, speed: 175, repairPerSec: 18, repairR: 70,
    update(s, dt) {
  ...
  soldier: {
    color: '#d24b35', label: 'SOL',
    hp: 100, speed: 175, fireRange: 320, fireCd: 0, damage: 18,
    update(s, dt) {
      s.fireCd -= dt;
      ...
      Game.bullets.push({
        ...
        vx: Math.cos(ang) * 900,
        vy: Math.sin(ang) * 900,
        life: 0.8, damage: s.damage, owner: 'squad', weapon: 'pistol',
      });
      s.fireCd = 0.55 + Math.random() * 0.2;
      if (Audio && Audio.sfx && Audio.sfx.pistol) Audio.sfx.pistol();
    },
  },
};
...
function spawnWorldSurvivor(x, y, cls, name, backstory) {
  ...
  const def = SQUAD_CLASS[cls];
  Game.worldSurvivors.push({
    x, y, r: 12, cls,
    name: name || SURVIVOR_NAMES[Math.floor(Math.random() * SURVIVOR_NAMES.length)],
    backstory: backstory || (SURVIVOR_BACKSTORIES[cls] || [''])[Math.floor(Math.random() * (SURVIVOR_BACKSTORIES[cls] || ['']).length)],
    hp: def.hp, maxHp: def.hp, walkPhase: Math.random(), angle: 0,
  });
}
...
function recruitSurvivor(s) {
  ...
  Game.squad.push({
    ...
    angle: 0, walkPhase: 0,
    holdMode: false, holdX: s.x, holdY: s.y,
    fireCd: 0, iframe: 0,
  });
